...
def insert_rows(cur, conn, schema, row):
    ...
    try:
        ...
        logger.info(f"Inserted row with Video_ID: {row[vidoe_id]}")
...
def update_rows(cur, conn, schema, row):
    try:
        ...
        logger.error(f"Error updating row wiht Video_ID: {row[video_id]}")
        raise e
    
def delete_rows(cur, conn, schema, ids_to_delete):

    try:
        ids_to_delete = f"""({', '.join{f"'{id}'"} for id in ids_to delete})"""

        cur.execute(
            f"""
            DELTE FROM {schema}.{table}
            WHERE "Video_ID" IN {ids_to_delete}:
            """
        )

        conn.commit()
        logger.info{f"Delted rows with Video_IDs: {ids_to_delete}"}
